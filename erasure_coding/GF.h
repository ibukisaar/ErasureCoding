...
private:
    ...
    static constexpr u8 value_table[]{
    ...
    constexpr static GF _mul(GF left, GF right) noexcept {
        if (left.value == 0 || right.value == 0) return 0;
        return from_exp(left.exp() + right.exp());
    }

    template<typename>
    struct _table_ex {};

    using table_ex = _table_ex<std::make_index_sequence<65536>>;

public:
    constexpr GF() noexcept : value(0) {}
    constexpr GF(u8 value) noexcept : value(value) {}

    constexpr static GF from_exp(int exp) noexcept {
        exp = exp % 255;
        if (exp < 0) exp += 255;
        return value_table[exp + 1];
    }

    constexpr friend GF operator +(GF left, GF right) noexcept {
        return left.value ^ right.value;
    }

    constexpr GF& operator +=(GF right) noexcept {
        value ^= right.value;
        return *this;
    }
    ...
    constexpr int exp() const noexcept { return exp_table[value]; }

    constexpr GF pow(int exp) const noexcept {
        if (exp == 0) return 1;
        if (value == 0) return 0;
        return from_exp(this->exp() * exp);
    }

    // 求逆。调用者需要自己检查等于0的情况。
    constexpr GF inverse() const noexcept {
        if (value <= 1) return value;
        return value_table[256 - exp()];
    }

    // 导出元素的乘法表。
    constexpr GF const* multiplier() const noexcept;
};

template<size_t...I>
struct GF::_table_ex<std::index_sequence<I...>> {
    static constexpr GF mul_table[]{
        _mul(I % 256, I / 256)... // 利用模板元编程在编译期求出乘法表
    };
};

constexpr GF operator *(GF left, GF right) noexcept {
    return GF::table_ex::mul_table[right.value * 256 + left.value];
}

constexpr GF const* GF::multiplier() const noexcept {
    return table_ex::mul_table + value * 256;
}